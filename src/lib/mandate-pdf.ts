...
function mandateDeclarationText(input: MandatePdfInput) {
  ...
  if (input.entityType === ClientEntityType.DECEASED_ESTATE) {
    return `I, ${input.clientName}, acting on behalf of ${entityName || "the deceased estate"}, hereby request License Hub's assistance in obtaining a duplicate vehicle registration document.`;
  }

  return "I hereby state that I have lost my vehicle's registration document and request License Hub's assistance in obtaining a duplicate vehicle registration document on my behalf.";
}
...
export async function createMandatePdf(input: MandatePdfInput) {
  ...
  const canRenderIdentityImage =
    Boolean(input.idPhotoBytes) &&
    (input.idPhotoMimeType === "image/jpeg" || input.idPhotoMimeType === "image/png");
  const idPhotoImage =
    canRenderIdentityImage && input.idPhotoBytes && input.idPhotoMimeType
      ? await embedImage(pdfDoc, input.idPhotoBytes, input.idPhotoMimeType)
      : null;
  ...
  drawCenteredText(page, "Uploaded identity document for verification", y - 52, 10, bold, navy);
  ...
  if (idPhotoImage) {
    ...
    drawCenteredText(page, "Identity document on file (preview unavailable)", photoBox.y + photoBox.height / 2, 11, bold, muted);
  }
