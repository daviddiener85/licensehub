...
import { listAdminReferrals, referralAmountPaid } from "@/lib/referrals";
...
export default async function AdminReferralsPage({
  ...
  const applications = await listAdminReferrals(query);
    ...
    <main className="min-h-screen bg-[#f7f5ef] text-[#1f2724]">
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-8 sm:py-8">
        ...
        <form className="mt-6 flex flex-col gap-3 border border-[#d8d1c3] bg-white p-4 sm:flex-row">
          <label className="flex-1 text-sm font-semibold">
            ...
            <input
              name="q"
              defaultValue={query}
              placeholder="Application, register number, client, cellphone, or referral name"
        ...
        <section className="mt-6 space-y-3 md:space-y-0 md:overflow-hidden md:border md:border-[#d8d1c3] md:bg-white">
          <div className="hidden grid-cols-[1.3fr_0.9fr_1.1fr_1fr_1.1fr_0.9fr] border-b border-[#d8d1c3] bg-[#fffdf8] px-4 py-3 text-xs font-semibold uppercase text-[#6b5e4f] md:grid">
          ...
          {applications.map((application) => (
            <div
              key={application.id}
              className="grid grid-cols-2 gap-x-3 gap-y-4 border border-[#d8d1c3] bg-white px-4 py-4 text-sm md:grid-cols-[1.3fr_0.9fr_1.1fr_1fr_1.1fr_0.9fr] md:items-center md:gap-2 md:border-x-0 md:border-t-0 md:border-b-[#eee8dc] md:last:border-b-0"
            >
          ...
          {applications.length === 0 ? (
            <div className="px-4 py-8 text-sm text-[#52615b]">No referrals found for this search.</div>
          ) : null}
        </section>
      </div>
    </main>
